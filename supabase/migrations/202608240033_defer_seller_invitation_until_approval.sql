...
create unique index if not exists seller_applications_open_email_idx
  on public.seller_applications (lower(contact_email))
...
begin
  ...
  set status = 'approved', admin_note = nullif(trim(p_admin_note), ''),
