...
const reviewSchema = z.discriminatedUnion("action", [
  ...
  z.object({
    ...
    missingField: z.string().trim().min(2).max(120),
  }),
  z.object({
  ...
  z.object({
    ...
    checklist: z.object({
      store_name_valid: z.boolean(),
      authorized_person_verified: z.boolean(),
      contact_verified: z.boolean(),
      company_verified: z.boolean(),
      documents_complete: z.boolean(),
      iban_verified: z.boolean(),
    }),
  }),
]);
...
const sellerPanelUrl = (
  ...
  "https://marketplace-platform-seller-panel.vercel.app"
).replace(/\/$/, "");
...
async function availableStoreSlug(
...
export async function PATCH(
  ...
  if (!actor) {
    return NextResponse.json(
      { error: "Satıcı başvurusu değerlendirme yetkiniz bulunmuyor." },
      { status: 403 },
    );
  }
  ...
  if (!parsed.success)
    return NextResponse.json(
      { error: "Değerlendirme bilgileri geçersiz." },
      { status: 400 },
    );
  ...
  const { data: application } = await adminClient
    ...
    .select(
      "id,applicant_user_id,store_name,status,contact_email,authorized_name,iban,document_status,review_checklist",
    )
    .eq("id", applicationId)
    .maybeSingle();
  if (!application)
    return NextResponse.json(
      { error: "Satıcı başvurusu bulunamadı." },
      { status: 404 },
    );

  if (parsed.data.action === "resend_invitation") {
    if (application.status !== "approved" || !application.applicant_user_id) {
      return NextResponse.json(
        {
          error:
            "Yeni şifre bağlantısı yalnızca onaylı satıcıya gönderilebilir.",
        },
        { status: 409 },
      );
    }
    ...
    if (error) {
      console.error("seller_invitation_resend_failed", {
        applicationId,
        code: error.code,
        status: error.status,
      ...
      return NextResponse.json(
        { error: "Yeni şifre bağlantısı gönderilemedi." },
        { status: 503 },
      );
    }
    await adminClient.from("admin_audit_logs").insert({
      ...
      action: `${application.store_name} satıcısına yeni şifre bağlantısı gönderildi`,
  ...
  if (parsed.data.action === "update_review") {
    if (application.status !== "under_review") {
      return NextResponse.json(
        {
          error:
            "Kontrol listesi yalnızca incelemedeki başvuruda güncellenebilir.",
        },
        { status: 409 },
      );
    }
    ...
    if (error)
      return NextResponse.json(
        { error: "Başvuru kontrol listesi kaydedilemedi." },
        { status: 500 },
      );
    await adminClient.from("admin_audit_logs").insert({
      actor_user_id: actor.userId,
      actor_user_code: actor.userCode,
      action: `${application.store_name} başvuru kontrolleri güncellendi`,
      module: "Satıcı Yönetimi",
      entity_type: "seller_application",
      entity_id: applicationId,
      risk: "warning",
  ...
  if (parsed.data.action === "approve") {
    if (application.status !== "under_review") {
      return NextResponse.json(
        { error: "Başvuru onaylanmadan önce incelemeye alınmalıdır." },
        { status: 409 },
      );
    }
    const checks = application.review_checklist as Record<
      string,
      boolean
    > | null;
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Yetkili, IBAN, evrak durumu ve tüm kontrol maddeleri tamamlanmadan başvuru onaylanamaz.",
        },
        { status: 409 },
      );
    }
    ...
    await adminClient.from("admin_audit_logs").insert({
      actor_user_id: actor.userId,
      actor_user_code: actor.userCode,
      action: `${application.store_name} satıcı başvurusu onaylandı`,
      module: "Satıcı Yönetimi",
      entity_type: "seller_application",
      entity_id: applicationId,
      risk: "warning",
  ...
  const allowedStatuses =
    parsed.data.action === "review" ? ["submitted"] : ["under_review"];
  const { data, error } = await adminClient
    ...
    .update({
      ...
      admin_note: parsed.data.note || null,
      ...(parsed.data.action === "request_revision" && {
        missing_field: parsed.data.missingField,
  ...
  if (!data)
    return NextResponse.json(
      { error: "Başvurunun durumu başka bir işlem tarafından değiştirilmiş." },
      { status: 409 },
    );

  const labels = {
    review: "incelemeye alındı",
    request_revision: "revizyona gönderildi",
    reject: "reddedildi",
  } as const;
  await adminClient.from("admin_audit_logs").insert({
    actor_user_id: actor.userId,
    actor_user_code: actor.userCode,
    action: `${application.store_name} başvurusu ${labels[parsed.data.action]}`,
    module: "Satıcı Yönetimi",
    entity_type: "seller_application",
    entity_id: applicationId,
