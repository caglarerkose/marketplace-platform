...
const updateSchema = z.object({
  permissions: z.array(z.enum(["view", "support", "product_approval"])).min(1).optional(),
...
export async function PATCH(
  ...
  if (parsed.data.permissions) {
    ...
    const { error } = await adminClient.from("admin_user_permissions").insert(
    ...
    if (error) return NextResponse.json({ error: "Yetkiler kaydedilemedi." }, { status: 500 });
  }
